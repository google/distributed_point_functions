...
#ifndef DISTRIBUTED_POINT_FUNCTIONS_PIR_SIMPLE_HASHING_SPARSE_DPF_PIR_CLIENT_H_
#define DISTRIBUTED_POINT_FUNCTIONS_PIR_SIMPLE_HASHING_SPARSE_DPF_PIR_CLIENT_H_
...
class SimpleHashingSparseDpfPirClient
    : public DpfPirClient<absl::Span<const std::string>,
                          std::vector<absl::optional<std::string>>> {
 public:
  // Creates a new SimpleHashingSparseDpfPirClient with the given `params` and
  ...
  static absl::StatusOr<std::unique_ptr<SimpleHashingSparseDpfPirClient>>
  ...
  // returns the database value at that key, or absl::nullopt if the key is not
  // present. The returned values will be padded with null bytes to the size of
  // the largest database entry. Returns INVALID_ARGUMENT if either the response
  // or the client state is invalid.
 ...
 private:
  SimpleHashingSparseDpfPirClient(
...
#endif  // DISTRIBUTED_POINT_FUNCTIONS_PIR_SIMPLE_HASHING_SPARSE_DPF_PIR_CLIENT_H_
